...
def _log(msg):
    print(f"[HR-MCP] {msg}", file=sys.stderr, flush=True)
...
mcp = FastMCP(
    ...
    instructions="AI 招聘系统面试时间收集服务 - 自动启动 Flask + Cloudflare 外网隧道"
)
...
if __name__ == "__main__":
    _log("HR Form Service MCP Server 启动中...")

    def _async_startup():
        results = startup()
        for r in results:
            _log(f"{r['service']}: {r['status']} - {r['message']}")
        if SERVICES_READY:
            _log(f"所有服务就绪！外网链接: {EXTERNAL_URL}")
        else:
            _log("部分服务启动失败，可调用 system_status 查看详情")

    threading.Thread(target=_async_startup, daemon=True).start()

    _log("MCP Server 等待客户端连接...")
